...
// Does not care about device data, use with care...
	template<typename T>
	T& MatrixIterator<T>::operator()(const size_t index)
	{
		return m_std_iterator[index];
	}

	template<typename T>
	T& MatrixIterator<T>::operator()(const size_t row, const size_t col)
	{
		m_parent->updateHost();
	...
	template<typename T>
	const T& MatrixIterator<T>::operator()(const size_t row, const size_t col) const
	{
		m_parent->updateHost();
		return m_std_iterator[(row*getParent().total_cols() + col)];
	}
	...
	template<typename T>
	MatrixIterator<T>& MatrixIterator<T>::stride_row(const size_t stride)
	{
	...
	template<typename T>
	const T& MatrixIterator<T>::operator[](const size_t index) const
	{
		m_parent->updateHost();
		return m_std_iterator[index];
	}
...
/*	template<typename T>
	...
	template<typename T>
	const MatrixIterator<T>& MatrixIterator<T>::operator+=(const size_t i)
	{
		m_std_iterator += i;
		return *this;
	}
	...
	template<typename T>
	MatrixIterator<T> MatrixIterator<T>::operator-(const size_t i) const
	{
		iterator temp(*this);
		temp -= i;
		return temp;
